...
import { isCjs, isJs, isTs } from "../../srcUtils.js";
...
export default class extends Generator<Options> {
  writing() {
    for (const srcSetName of srcSetNames) {
      ...
      const moduleType = isCjs(srcSetName) ? "commonjs" : "module";
      this.fs.copyTpl(
      ...
      this.fs.copy(
        this.templatePath(fromCurrentDir(".swcrc")),
        this.destinationPath(fromRootDir(`generated/${pkgName}/.swcrc`))
      );
      this.fs.copyTpl(
        this.templatePath(fromCurrentDir("jsconfig.json.ejs")),
        this.destinationPath(fromRootDir(`generated/${pkgName}/jsconfig.json`))
      );
    }
  }
}
